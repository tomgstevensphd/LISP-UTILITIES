...
;;ddd
;;sss START HERE -- LATER FIX THE ASCEND SORT PROBLEM IN U-LISTS
(defun organize-subdirs (main-dir sort-type)
  ...
  (let
    ...
    (multiple-value-setq (flat-subdirs-list simple-all-files-list)
        (make-flat-dirs-list main-dir t))
    (show-text (format nil "In organize-subdirs, flat-subdirs-list= ~A~%" flat-subdirs-list)400 t)
    ...
    (cond
     ...
     ((equal sort-type 'greaterp)
      (setf a t)))
    (setq organized-dir-list 
           (organize-sublists flat-subdirs-list :randomize  r  :descend-sort d  :ascend-sort a))
    (values organized-dir-list flat-subdirs-list simple-all-files-list)))



;; Test  works
(defun testmfd ()
  (make-flat-dirs-list  "C:\\TOM\\LISP PROJECTS\\MyProjects\\screensaver" t)
  ;;was "c:\\temp")
  )

;;MAKE-FLAT-DIRS-LIST
;;works--look at make-flat-dirs-list OUTPUT.docx file for sample output
;;ddd
(defun make-flat-dirs-list (dir &optional return-path-objects)
  ...
  (let
    ...
    ;;   (show-text (format nil "dir-list= ~A~%" dir-list) 100 t)
    ;;returns dir-flat-list with EXTRA PARENS ENCLOSING EACH LOWER LEVEL
    (multiple-value-setq (dir-flat-list sub-dir-list)
        (make-flat-dirs-list1 dir 0 nil return-path-objects ))
    ;;make completely flat list with no extra parens for lower levels
    (multiple-value-setq (dir-sublists dir-complete-flat-list )
        (flatten-list-tree dir-flat-list))
    (show-text (format nil "In make-flat-dirs-list, FINAL, dir-flat-list= ~A~% sub-dir-list= ~A~%dir-sublists= ~A~%dir-complete-flat-list= ~A~%"dir-flat-list dir-sublists dir-complete-flat-list sub-dir-list) 600 t)     
    (values dir-flat-list dir-complete-flat-list )))
...
;;ddd
;; (probe-file  "C:\\TOM\\LISP PROJECTS\\MyProjects\\screensaver")=>
;;  #P"C:/TOM/LISP PROJECTS/MyProjects/screensaver/"

;;sss START HERE AND JUST DEBUG  PHOTO-VIEWER
;;
(defun make-flat-dirs-list1 (dir-or-dir-list N &optional dir-flat-list return-path-objects)
  "In U-files.lisp, works with make-flat-dirs-list as recursor"
  (let
      ((sub-dir-list)
       (sub-dir-name-list)
       (sub-file-list)
    ...
    (if (not (listp dir-or-dir-list))
             (setf sub-dir-list (list (list-directory dir-or-dir-list))))

    (dolist (subdir sub-dir-list)
      #| (show-text (format nil "1-IN MAKE-FLAT-DIRS-LIST1, dir-or-dir-list ~A~%"
                         dir-or-dir-list) 100 t)|#
      ;;separate the files at current folder level from subdirs in same folder
      (multiple-value-setq (sub-file-list sub-dir-list)
          (separate-files-from-dirs subdir))
      ;;add sub-file-list to dir-flat-list (lists of files from each sub-dir)
      (setf dir-flat-list (append dir-flat-list (list sub-file-list)))
      ;;recurse function
      #|      (show-text 
       (format nil "N= ~A~% sub-file-list = ~A~%sub-dir-list= ~A~% dir-flat-list=~A~% "
                         n sub-file-list sub-dir-list dir-flat-list) 200 t)|#
      (cond
       ;; this is the limit to recursion calls
       ((>  n 20) nil)
       ((> (length sub-dir-list) 0)
        (multiple-value-bind (return-flat-dir-list return-sub-dir-list)
            ;;zzz
            (make-flat-dirs-list1 sub-dir-list n nil t) ;;try not sending current flat list down dir-flat-list)
          ;;added return here to exit more directly and omit show-text
          (setf dir-flat-list (append dir-flat-list (list return-flat-dir-list)))
          (setf  sub-dir-list return-sub-dir-list)
          ))
       ;;doesn't work?   (return  (values dir-flat-list sub-dir-list)))
       (t nil))
      ;;end dolist
      )
    #|    (show-text 
     (format nil "2-In MAKE-FLAT-DIRS-LIST1, N= ~A~% dir-flat-list= ~A~%~% sub-dir-list ~A~%" n dir-flat-list sub-dir-list) 100 t)|#
    ...
    (dolist (element dir-list)
     ;; (show-text (format nil "element= ~A~%" element) 20 t)
      ...
      ;;  (show-text (format nil "file-name= ~A~%" element) 20 t)
       ...
       (t nil)))
   #|   (show-text 
       (format nil "2-In separate-files-from-dirs,, file-list= ~A~%~% sub-dir-list= ~A~%"
               file-list sub-dir-list ) 200 t)|#
