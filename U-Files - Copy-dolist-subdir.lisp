...
;;sss START HERE DEBUGGING THE ACCUMULATION OF NILS -- SEE FOUT
...
#| (afout '*ufiles-out (format nil "1 XX-In make-flat-dirs-list, FINAL, dir-almost-flat-list= ~A~% sub-dir-list= ~A~%dir-sublists= ~A~% simple-all-files-list= ~A~%"dir-almost-flat-list dir-sublists simple-all-files-list sub-dir-list))  600 t) |#

;;    (multiple-value-setq (dir-flat-list simple-all-files-list)
 ;;       (flatten-list-tree dir-almost-flat-list))
...
;;sss START HERE DEBUGGING EXTRA PAREN IN REST OF FLAT SUBLISTS
;; NOTE:  AT FINAL, flat-list= THE LIST DOESN'T HAVE THE EXTRA PAREN, 
;;   BUT DOES AT THE REST OF THE SHOW-TEXTS

;;ddd
(defun make-flat-dirs-list1 (dir-or-dir-list N &optional dir-flat-list)
  ...
  (let
    ...
    (incf n)
    ;;put in form of a list for dolist below
   ;; unquote this later??
   ;;(if (not (listp dir-or-dir-list)) (setf dir-or-dir-list (list dir-or-dir-list)))

 (afout '*ufiles-out (format nil "0-IN MAKE-FLAT-DIRS-LIST1, dir-or-dir-list ~A~%"
                         dir-or-dir-list)) ;; 100 t)

    ;;for each element in the list -- may be file or dir or subdir
    (dolist (subdir dir-or-dir-list)
 ...
 (afout '*ufiles-out  (format nil "2-IN MAKE-FLAT-DIRS-LIST1, 
                       dir-flat-list ~A~%sub-dir-list= ~A~%" dir-flat-list sub-dir-list)) ;; 100 t)
#|     (cond
       ((= n 1)
        (setf dir-flat-list (list sub-file-list)))
       ((= n 2)
        (setf dir-flat-list (append dir-flat-list (list sub-file-list))))
       (t               
        (setf dir-flat-list (append dir-flat-list (list sub-file-list)))))|#
...
#|      (afout *ufiles-out 
      ...
      (cond
       ...
       ((> (length sub-dir-list) 0)
        ;;separate list of files at this level from subdirs at this level
        (multiple-value-setq (sub-file-list sub-dir-list)
            (separate-files-from-dirs subdir))

        ;;start recursion into subdirs at multiple levels
        (multiple-value-bind (return-dir-flat-list return-sub-dir-list)
            (make-flat-dirs-list1 sub-dir-list n dir-flat-list) 
          (setf dir-flat-list (append dir-flat-list (list return-dir-flat-list)))
     ...
     (afout '*ufiles-out  (format nil "In make-flat-dirs-list1 N= ~A~% dir-flat-list= ~A~% sub-dir-list= ~A~%" N dir-flat-list sub-dir-list) ) ;;120 t)
