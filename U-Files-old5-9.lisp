...
(defun make-flat-dirs-list1 (dir-list N &optional dir-flat-list)
  ...
  (let
    ...
    (afout '*ufiles-out (format nil "0-IN MAKE-FLAT-DIRS-LIST1, dir-list ~A~%"
                                dir-list)) ;; 100 t)

    ;;for each element in the list -- may be file or dir or subdir
    (dolist (element dir-list)
      (afout '*ufiles-out
             (format nil "1-IN MAKE-FLAT-DIRS-LIST1,element= ~A~% dir-list= ~A~%"                 element dir-list))
      (cond
       ((>  n *recursion-limit) nil)
       ;;tests to see if it is a directory or a file- returns file list if a dir
       ((and (setf sub-dir-list (list-directory element))
        ...
        (multiple-value-setq (sub-file-list sub-dir-list)
            (separate-files-from-dirs element))
       ...
       (t (setf sub-file-list (append sub-file-list (list element)))
    ...
    (afout '*ufiles-out  (format nil "3-In make-flat-dirs-list1 N= ~A~% dir-flat-list= ~A~% sub-dir-list= ~A~%" N dir-flat-list sub-dir-list) ) ;;120 t)
